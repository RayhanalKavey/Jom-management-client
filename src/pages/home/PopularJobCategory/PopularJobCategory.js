...
const PopularJobCategory = () => {
  const jobCategory = [
    ...
    {
      postition: "Accounting and Finance",
      opening: "59 opening",
      icon: (
        <svg
          xmlns="http://www.w3.org/2000/svg"
    ...
    <div className=" grid  sm:grid-cols-2 md:grid-cols-3 gap-4 w-full ">
      {jobCategory?.slice(0, 6)?.map((category, i) => (
        <div
          ...
          key={i}
        >
          <div className=" iconStyle rounded-full bg-success p-2 ">
            {category?.icon}
          </div>
          <div className="flex-2 flex items-center sm:items-start flex-col">
            {" "}
            <div className="text-xl font-semibold mb-5">
              {category?.postition}
            </div>
            <div className="font-info">({category?.opening})</div>
          </div>
        </div>
      ))}
    </div>
